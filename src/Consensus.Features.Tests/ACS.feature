Feature: Active Contract Set
  ...
  Scenario: A contract is activated
    Given genesisTx locks 10 metavcoin to key1
  ...
  Scenario: A contract is reactivated
    ...
    And reactivationTx locks change metavcoin to activationTxChangeKey1
  ...
  Scenario: An expired contract is not extended
    ...
    And extensionTx locks change metavcoin to extensionTxChangeKey1
  ...
  Scenario: A contract is activated and executed in main chain
    Given genesisTx locks 10 metavcoin to key1
    And genesisTx locks 10 metavcoin to key2
    And genesis has genesisTx
    And activationTx has the input genesisTx index 0
    And activationTx activates c1 for 1000 blocks
    And activationTx locks change metavcoin to activationTxChangeKey
    ...
    Given txMain locks 10 metavcoin to mainKey3
  ...
  Scenario: Complex reorgs
    ...
    And genesisTx locks 10 metavcoin to key2
    And genesisTx locks 10 metavcoin to key2
    And genesisTx locks 10 metavcoin to key3
    And genesisTx locks 10 metavcoin to key4
    And genesis has genesisTx
    ...
    Given txMainBis locks 10 metavcoin to mainBisKey3
