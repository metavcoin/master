...
module Wallet =
    ...
    let metavcoinPublicKey
        (config: Config)
        : unit =
            match Wallet.exportmetavcoinPublicKey config.client with
            | Ok publicKey ->
    ...
    let importmetavcoinPublicKey
        (config: Config)
        (body: string)
        : unit =
            match parseImportmetavcoinPublicKey body with
            | Error error -> config.replyError error
            | Ok publicKey ->
                match Wallet.importmetavcoinPublicKey config.client publicKey with
                | Ok _ ->
                    config.reply StatusCode.OK (TextContent "metavcoinKey imported")
                | Error error -> config.replyError error
...
let handleRequest (chain:Chain) client (request,reply) (templateCache : BlockTemplateCache) (isRemote:bool) =
    ...
    | Get("/wallet/metavcoinpublickey", _) ->
        Wallet.metavcoinPublicKey config
    | Post("/wallet/importmetavcoinpublickey", Some body) ->
       Wallet.importmetavcoinPublicKey config body
