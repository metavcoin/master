...
let setUp = fun () ->
    ...
    let redeem txSkeleton contractId returnAddress wallet =
      ...
      RT.ofOption "contract doesn't have enough metavcoins tokens" result
    ...
    let cf _ _ _ _ _ wallet _ =
        4 + 64 + 2 + (64 + (64 + (64 + 64 + (metavcoin.Wallet.size wallet * 128 + 192) + 0)) + 33) + 31
        |> cast nat
        |> C.ret
...
let ``Should buy``() =
    ...
    let spend = { asset = metavcoin; amount = 5UL }
...
let ``Should redeem``() =
    ...
    let utxoSet =
        Map.empty
        |> Map.add inputmetavcoin (UtxoSet.Unspent outputmetavcoin)
    ...
    | Ok tx ->
        ...
        tx.inputs |> should contain (Outpoint inputmetavcoin)
        tx.outputs |> should haveLength 2
        tx.outputs |> should contain { lock = Destroy; spend = spendContractAsset }
        tx.outputs |> should contain { lock = PK samplePKHash; spend = spendmetavcoin }
        tx.witnesses |> should haveLength 1
        let wit, cost =
            match tx.witnesses.[0] with
            | ContractWitness {cost=cost} as wit ->
