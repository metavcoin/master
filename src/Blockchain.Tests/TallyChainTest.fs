    ...
    let signatureData =
        let map =
            ...
            |> List.map (fun (publicKeyAsString, pathAsString) ->
                ...
                publicKey, signature |> ZFStar.fsToFstSignature |> metavcoin.Types.Data.Signature
                )
            |> Map.ofList
        metavcoin.Types.Data.Collection (metavcoin.Types.Data.Dict (map, Map.count map |> uint32))
    let signatures =
        metavcoin.Dictionary.add signatureString signatureData (Map.empty, 0ul)
        |> metavcoin.Cost.Realized.__force

    metavcoin.Dictionary.add ballotString (metavcoin.Types.Data.String ballotSer) signatures
    |> metavcoin.Cost.Realized.__force
    |> metavcoin.Types.Data.Dict
...
let createVoteTransaction session (account:TestWallet.T) blockNumber timestamp ballot =
    ...
    let executeContract (contractId:ContractId) command sender (messageBody: metavcoin.Types.Data.data option) (txSkel : TxSkeleton.T) =
