...
let contract2Code = """
// contract 2: this contract receives the remaining tx metavcoin tokens and locks them to itself
...
let main txSkeleton _ contractId command sender messageBody wallet state =
    ...
    begin
        ...
        let! txSkeleton =
            Tx.lockToContract metavcoinAsset tokens contractId txSkeleton in
...
let contract1Code =
    ...
    |> sprintf """
// contract 1: this contract receives metavcoin tokens; mints and locks it's own tokens to the return-address, and passes a message to contract 2
...
module RT = metavcoin.ResultT
module Tx = metavcoin.TxSkeleton
module ContractId = metavcoin.ContractId
module C = metavcoin.Cost
