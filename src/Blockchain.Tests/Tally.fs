...
let ``validatePayout tests`` () =
     ...
     let env = { env with lastFund = Map.empty |> Map.add Asset.metavcoin 1UL }
     ...
     let vote = (someone, [{asset=Asset.metavcoin; amount=1UL}; {asset=asset0; amount=1UL}])
     validatePayout env vote
     |> ``should be`` (Some vote)
     
     let vote = (someone, [{asset=Asset.metavcoin; amount=1UL}; {asset=asset0; amount=5UL}])
     validatePayout env vote
     |> ``should be`` (Some vote)
