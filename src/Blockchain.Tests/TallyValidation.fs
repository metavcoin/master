...
let ``validatePayout tests`` () =
     ...
     let vote = (someone, [{asset=Asset.metavcoin; amount=0UL}])
     validatePayout env vote
     |> ``should be`` None
     
     let vote = (someone, [{asset=Asset.metavcoin; amount=1UL}; {asset=asset0; amount=6UL}])
     validatePayout env vote
     |> ``should be`` None
     ...
     let env = { env with
                    nomineesBallots = Map.add genPk (someone, [{asset=Asset.metavcoin; amount=1UL }]) Map.empty
                    balances        = Map.add genPkHash 10000UL Map.empty
                  }
     
     let vote = (cgpContract, [{asset=Asset.metavcoin; amount=1UL}])
     validatePayout env vote
     |> ``should be`` (Some vote)
     
     let env = { env with lastFund = Map.empty |> Map.add Asset.metavcoin 2UL }
     
     let vote = (someone, [{asset=Asset.metavcoin; amount=1UL}])
     validatePayout env vote
     |> ``should be`` (Some vote)
     ...
     let env = { env with
                         lastFund =
                              Map.empty
                              |> Map.add Asset.metavcoin 2UL
                              |> Map.add asset0 5UL
                         nomineesBallots =
                              Map.empty
                              |> Map.add genPk (someone, [{asset=Asset.metavcoin; amount=2UL}])
                         balances        = Map.add genPkHash 10000UL Map.empty
                              }
     ...
     let vote = (someone, [{asset=Asset.metavcoin; amount=2UL}])
     validatePayout env vote
     |> ``should be`` (Some vote)
     
     let vote = (someone, [{asset=Asset.metavcoin; amount=1UL}; {asset=Asset.metavcoin; amount=2UL}])
     validatePayout env vote
     |> ``should be`` None
     ...
     let env = { env with
                    ...
                    nomineesBallots =
                         Map.empty
                         |> Map.add genPk (someoneElse, [{asset=Asset.metavcoin; amount=1UL}; {asset=asset0; amount=5UL}])
                    balances        = Map.add genPkHash 10000UL Map.empty
                         }
     
     let vote = (someoneElse, [{asset=Asset.metavcoin; amount=1UL}; {asset=asset0; amount=5UL}])
     validatePayout env vote
     |> ``should be`` (Some vote)
...
let ``Payout votes are correctly tallied`` () =
     ...
     let vote = (someoneElse , [{asset=Asset.metavcoin; amount=1UL}])
