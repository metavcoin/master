...
let initOutputDir moduleName =
    ...
    let oDir =
        if unitTesting then
            ...
            |> Platform.normalimetavcoinameToFileSystem
        else
            Platform.normalimetavcoinameToFileSystem compatibilityPath
#else
    let oDir = Platform.normalimetavcoinameToFileSystem (Path.GetTempPath()) / Path.GetRandomFileName()
#endif
