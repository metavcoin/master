...
type Env =
    {
        ...
        getContractState : ContractId -> metavcoin.Types.Data.data option
...
module Coinbase =
    ...
    let private computeblockRewardAndFees
        ...
        let totalmetavcoin =
            Fund.find Asset.metavcoin blockFees
        ...
        Map.add Asset.metavcoin (totalmetavcoin + blockSacrifice + blockReward + allocationReward) blockFees
    ...
    let check
        ...
        if isGenesis env.chainParams env.block then
            ...
            let cgpmetavcoinAmount =
                Fund.find Asset.metavcoin cgpAmounts
            ...
            elif cgpmetavcoinAmount <> (blockAllocation env.block.header.blockNumber state.cgp.allocation) then
