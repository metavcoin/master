...
let deserialimetavcoinonZero fn bytes =
...
module Serialization =
    ...
    module Asset =
        ...
        let read stream =
            ...
            // Fast path for metavcoin asset, in case first byte is zero
            if first = 0uy then
                Asset.metavcoin
            else
...
module RawTransaction =
    ...
    let deserialize = deserialimetavcoinonZero RawTransaction.read
...
module Transaction =
    ...
    let deserialize mode = deserialimetavcoinonZero (Transaction.read mode)

module TransactionExtended =
    let serialize = serialize TransactionExtended.size TransactionExtended.write
    let deserialize = deserialimetavcoinonZero TransactionExtended.read

module TransactionsRaw =
    let serialize txs = Array.concat txs

module TransactionsExtended =
    let deserialize count = deserialimetavcoinonZero (List.readBody count TransactionExtended.read)
