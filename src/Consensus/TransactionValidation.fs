...
let private activateContract (chainParams : Chain.ChainParameters) contractPath blockNumber acs contractCache (tx : Types.Transaction) =
    let getActivationSacrifice tx = result {
        ...
        let allUsingmetavcoin =
            List.forall (fun output -> output.spend.asset = Asset.metavcoin) activationSacrifices

        if not allUsingmetavcoin then
            return! GeneralError "Sacrifice must be paid in metavcoin"
...
let private extendContracts chainParams acs tx = result {
    ...
    if List.exists (fun output -> output.spend.asset <> Asset.metavcoin) extensionSacrifices then
        return! GeneralError "Sacrifice must be paid in metavcoin"
