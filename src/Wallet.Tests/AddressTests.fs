...
[<Property>]
let ``Encoded Mainnet PK address should start with metavcoin``(ValidHash hash) =
    let address = Address.encode Main (PK hash)
    address.StartsWith("metavcoin")
...
[<Property>]
let ``Encoded Mainnet Contract address should start with cmetavcoin``version (ValidHash hash) =
    let address = Address.encode Main (Contract <| ContractId (version,hash))
    address.StartsWith("cmetavcoin")
