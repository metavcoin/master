...
let fundAccount session =
    let output = {lock = PK accountPKHash; spend={asset=Asset.metavcoin;amount=10000000UL}}
...
let ``received tokens - mempool``() =
    ...
    let output = {lock = PK accountPKHash; spend={asset=Asset.metavcoin;amount=10UL}}

    let tx = {version=Version0;inputs=[];outputs=[output];witnesses=[];contract=None}

    let view' = View.addMempoolTransaction dataAccess session (Transaction.hash tx) tx View.empty

    view' |> balanceShouldBe session Asset.metavcoin 10UL
...
let ``received tokens - block``() =
    ...
    balanceShouldBe session Asset.metavcoin 10UL View.empty
...
let ``tokens spent - mempool``() =
    ...
    balanceShouldBe session Asset.metavcoin 10UL view'
...
let ``creating, not enough tokens``() =
    ...
    let result = TransactionCreator.createTransaction testParameters dataAccess session view password [ { lock = (PK Hash.zero); spend = { asset = Asset.metavcoin; amount = 11UL } } ]
...
let ``creating, no change``() =
    ...
    let output = {lock = lockToBob; spend={asset=Asset.metavcoin;amount=10UL}}
    let tx = {version=Version0;inputs=[];outputs=[output];witnesses=[];contract=None}
    let bob = View.addMempoolTransaction dataAccess session (Transaction.hash tx) tx View.empty

    // sending money to alice
    let result = TransactionCreator.createTransaction testParameters dataAccess session bob password [ { lock = lockToAlice; spend = { asset = Asset.metavcoin; amount = 10UL } } ]
    ...
    | Ok tx ->
        ...
        bob |> balanceShouldBe session Asset.metavcoin 0UL
...
let ``creating, with change``() =
    ...
    // sending money to alice
    let result = TransactionCreator.createTransaction testParameters dataAccess session bob password [ { lock = lockToAlice; spend = { asset = Asset.metavcoin; amount = 7UL } } ]
    ...
    | Ok tx ->
        ...
        bob |> balanceShouldBe session Asset.metavcoin 3UL
...
let ``picking the correct asset``() =
    ...
    let output2 = {lock = lockToBob; spend={asset=Asset.metavcoin;amount=10UL}}
    ...
    bob |> balanceShouldBe session Asset.metavcoin 10UL
...
let ``picking from multiple inputs``() =
    ...
    // giving some money to bob
    let output = {lock = lockToBob; spend={asset=Asset.metavcoin;amount=5UL}}
    let output2 = {lock = lockToBob; spend={asset=Asset.metavcoin;amount=7UL}}
    ...
    match result with
        ...
        | Ok tx ->
            ...
            bob |> balanceShouldBe session Asset.metavcoin 2UL
...
let ``create execute contract transaction``() =
    ...
    let executeContract _ _ _ _ txSkeleton =
        let tx =
            txSkeleton
            |> TxSkeleton.addOutput {lock=Contract <| ContractId  (Version0,Hash.zero);spend={asset=Asset.metavcoin;amount=1UL}}
            |> Transaction.fromTxSkeleton

        tx |> Ok

    let spends = Map.add Asset.metavcoin 1UL Map.empty
...
[<Test>]
let ``create execute contract transaction without explicitly spending any metavcoin should allocate fee``() =
    use session = DataAccess.DatabaseContext.createSession databaseContext
    let view = fundAccount session
    ...
    tx.outputs
    |> List.exists (fun output -> output.lock = Fee && output.spend = { asset = Asset.metavcoin; amount = 1UL })
    |> should equal true
...
let ``wallet won't spend coinbase if not mature enough``() =
    ...
    let origin =
        {
            version=Version0
            inputs=[]
            outputs=[{lock =  Coinbase (1ul, accountPKHash); spend= {asset = Asset.metavcoin;amount=100000000UL}}]
            witnesses=[]
            contract=None
        }
    ...
    TransactionCreator.createTransaction testParameters dataAccess session view password [ { lock = (PK accountPKHash); spend = { asset = Asset.metavcoin; amount = 1UL } } ]
...
let ``wallet spend coinbase when come from block``() =
    ...
    TransactionCreator.createTransaction testParameters dataAccess session View.empty password [ { lock = (PK accountPKHash); spend = { asset = Asset.metavcoin; amount = 1UL } } ]
    |> should be ok
...
let ``Should get expected history``() =
    ...
    let output1 = {lock = PK accountPKHash; spend={asset=Asset.metavcoin;amount=10UL}}
    let tx1 = {version=Version0;inputs=[];outputs=[output1];witnesses=[];contract=None}
    let tx1Hash = Transaction.hash tx1
    let output2A = {lock = PK Hash.zero; spend={asset=Asset.metavcoin;amount=2UL}}
    let output2B = {lock = PK accountPKHash; spend={asset=Asset.metavcoin;amount=8UL}}
    ...
    let output3A = {lock = PK Hash.zero; spend={asset=Asset.metavcoin;amount=3UL}}
    let output3B = {lock = PK accountPKHash; spend={asset=Asset.metavcoin;amount=5UL}}
    let tx3 = {version=Version0;inputs=[Outpoint { txHash = tx2Hash; index = 1ul } ];outputs=[output3A;output3B];witnesses=[];contract=None}

    let expected = [
        (tx2Hash, TransactionDirection.Out, {asset=Asset.metavcoin;amount= 2UL}, 1u, PK accountPKHash);
        (tx1Hash, TransactionDirection.In,{asset=Asset.metavcoin;amount= 10UL}, 1u, PK accountPKHash) ]
    ...
    let expected = (tx3Hash, TransactionDirection.Out, {asset=Asset.metavcoin;amount= 3UL}, 0u, PK accountPKHash) :: expected
...
let ``Typescript testvector``() =
    ...
    let output = {lock = PK (Hash.zero);spend= {amount=100UL;asset=Asset.metavcoin}}
...
let ``creating and signing raw transaction``() =
    ...
    // sending money to alice
    let raw =
        TransactionCreator.createRawTransaction testParameters dataAccess session bob None [ { lock = lockToAlice; spend = { asset = Asset.metavcoin; amount = 7UL } } ]
