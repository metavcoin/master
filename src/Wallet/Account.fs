...
let metavcoinKeyPath = "m/44'/258'/0'/"
...
let getKeyPath addressType =
    ...
    | External index -> sprintf "%s0/%d" metavcoinKeyPath index
    | Change index -> sprintf "%s1/%d" metavcoinKeyPath index
    | Payment index -> sprintf "%s2/%d" metavcoinKeyPath index
...
let import dataAccess session mnemonicPhrase password tipHash tipBlockNumber = result {
    ...
    let! publicKey =
        ...
        >>= ExtendedKey.derivePath metavcoinKeyPath
        >>= ExtendedKey.neuter
...
let frommetavcoinPublicKey dataAccess session tipHash tipBlockNumber publicKey = result {
    let! externalPKHash =
            deriveExternal 0 publicKey
            >>= ExtendedKey.getPublicKey
            <@> PublicKey.hash
...
let getmetavcoinExtendedPublicKey dataAccess session =
