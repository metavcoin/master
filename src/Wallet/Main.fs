...
let requestHandler chain client (requestId:RequestId) request dataAccess session accountStatus =
    ...
    | NoAccount ->
        ...
        | ImportmetavcoinPublicKey b58 ->
            ...
            | Ok publicKey ->
                ...
                match Account.frommetavcoinPublicKey dataAccess session tipHash tipBlockNumber publicKey with
                | Error error ->
                    reply<unit> requestId (Error error)
                    NoAccount
                | Ok () ->
                    eventX "Account imported from metavcoin public key"
                    |> Log.info
    ...
    | Exist view ->
        ...
        | GetAddressBalance (address,confirmations) ->
            ...
            accountStatus
        | ExportmetavcoinPublicKey ->
            Account.getmetavcoinExtendedPublicKey dataAccess session
