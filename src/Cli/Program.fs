...
type Arguments =
    ...
    | [<CliPrefix(CliPrefix.None)>] ExportmetavcoinPublicKey of ParseResults<NoArgs>
    | [<CliPrefix(CliPrefix.None)>] ImportmetavcoinPublicKey of ParseResults<ImportPublicKeyArgs>
    ...
    interface IArgParserTemplate with
        member arg.Usage =
            ...
            | Local _ -> "use local"
#endif
            | Port _ -> "port of metavcoin-node API"
            ...
            | ExportmetavcoinPublicKey _ -> "export metavcoin extended public key"
            | ImportmetavcoinPublicKey _ -> "import metavcoin extended public key and create watch-only account"
...
[<EntryPoint>]
let main argv =
    let parser = ArgumentParser.Create<Arguments>(programName = "metavcoin-cli", errorHandler = errorHandler)
    ...
    try
        ...
        | Some (MnemonicPhrase _) ->
            ...
            |> printResponse
        | Some (ImportmetavcoinPublicKey args) ->
            let publicKey = args.GetResult <@ ImportPublicKey_Arguments @>
            importmetavcoinPublicKey getUri publicKey
            |> printResponse
        | Some (ExportmetavcoinPublicKey _) ->
            exportmetavcoinPublicKey getUri
            |> printResponse
        | Some (RemoveWallet _) ->
            let password = readPassword()
