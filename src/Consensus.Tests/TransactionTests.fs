...
let ``Transaction should be orphan``() =
    ...
    let output = { lock = PK testHash; spend = { asset = Asset.metavcoin; amount = 1UL } }
    let tx = {
        version = Version0
...
let ``Transaction validation should fail with outputs invalid error``() =
    ...
    check { tx with inputs = [ Outpoint testInput1 ];
                    outputs = [ { lock = PK testHash; spend = { asset = Asset.metavcoin; amount = 0UL } } ] }
...
let ``Transaction validation should fail with duplicate inputs error``() =
    ...
    check { tx with inputs = [ Outpoint testInput1; Outpoint testInput1 ]
                    outputs = [ { lock = PK testHash; spend = { asset = Asset.metavcoin; amount = 1UL } } ]
                    witnesses = [ witness ] }
...
let ``Transaction validation should fail with structurally invalid input data error``() =
    ...
    let tx = { tx with outputs = [ { lock = PK validHash; spend = { asset = Asset.metavcoin; amount = 10UL } } ] }
    // empty imputs
...
let ``Transaction validation should fail with structurally invalid output data``() =
    ...
    for identifier in [ 1u .. 7u ] do
        check { tx with outputs = [ { lock = HighVLock (identifier, [| byte 0 |]); spend = { asset = Asset.metavcoin; amount = 10UL } } ] }

    // missing highvlock bytes
    check { tx with outputs = [ { lock = HighVLock (100u, [||]); spend = { asset = Asset.metavcoin; amount = 10UL } } ] }

    check { tx with outputs = [ { lock = PK Hash.zero; spend = { asset = Asset (ContractId (Version0, Hash.zero), validHash); amount = 10UL } } ] }

    // zero amount
    check { tx with outputs = [ { lock = PK validHash; spend = { asset = Asset.metavcoin; amount = 0UL } } ] }

    // test with malformatted HighV lock
    for invalidArr in [ emptyArr; nullArr ] do
        check { tx with outputs = [ { lock = HighVLock (0ul, invalidArr); spend = { asset = Asset.metavcoin; amount = 10UL } } ] }
...
let ``Transaction validation should fail with structurally invalid witness data``() =
    let check witness =
        { tx with witnesses = [ witness ];
                  ...
                  outputs = [ { lock = PK validHash; spend = { asset = Asset.metavcoin; amount = 10UL } } ] }
...
let ``Signed transaction should be valid``() =
    ...
    let output = { lock = outputLock; spend = { asset = Asset.metavcoin; amount = 1UL } }
    let tx = {
        version = Version0
        inputs = [ Outpoint testInput1 ]
        witnesses = []
...
let ``sign with FollowingWitnesses sighash``() =
    ...
    let output1 = { lock = outputLock1; spend = { asset = Asset.metavcoin; amount = 1UL } }

    let outputLock2 = PK (PublicKey.hash publicKey2)
    let output2 = {lock = outputLock2; spend = { asset = Asset.metavcoin; amount = 1UL } }

    let tx = {
        version = Version0
        inputs = [ Outpoint testInput1;Outpoint testInput2]
        witnesses = []
        outputs = [ {lock = PK Hash.zero; spend = {asset = Asset.metavcoin; amount = 2UL}} ]
        contract = None
    }
...
let ``sign last input with FollowingWitnesses sighash and add another witness``() =
    ...
    let outputs = List.append signedTx.outputs [{lock = PK Hash.zero; spend = {asset = Asset.metavcoin; amount = 1UL}}]
