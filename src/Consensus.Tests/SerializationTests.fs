...
[<Test>]
let ``Asset round trips when metavcoin native token``() =
    Asset.metavcoin |> serializeAsset |> deserializeAsset |> Option.get
    |> should equal Asset.metavcoin
...
[<Test>]
let ``Asset has size 1 for metavcoin native token``() =
    Asset.metavcoin |> serializeAsset |> Array.length
    |> should equal 1
