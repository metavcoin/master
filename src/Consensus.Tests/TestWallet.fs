...
module ZData = metavcoin.Types.Data
module Cost = metavcoin.Cost.Realized
...
type T = {
    ...
    blockNumber: uint32
    publicKey: PublicKey
    publicKeyChange: PublicKey
    cgp: CGP.T
}

let private metavcoinCoinType = ExtendedKey.Hardened 258
let private purpose = ExtendedKey.Hardened 44

let derivemetavcoinKey = ExtendedKey.derivePath "m/44'/258'/0'/0/0"

let deriveChangemetavcoinKey = ExtendedKey.derivePath "m/44'/258'/0'/1/0"
...
let private create mnemonicPhrase password tipHash tipBlockNumber = result {
    ...
    let! metavcoinPrivateKey = derivemetavcoinKey extendedPrivateKey
    let! publicKey =ExtendedKey.getPublicKey metavcoinPrivateKey

    let! metavcoinChangePrivateKey = deriveChangemetavcoinKey extendedPrivateKey
    let! publicKeyChange =ExtendedKey.getPublicKey metavcoinChangePrivateKey
...
let private isKeyMatch account address =
    PublicKey.hash account.publicKey = address || PublicKey.hash account.publicKeyChange = address
...
let private collectInputs chainParams account amount asset secretKey secretChangeKey =
    let unspent, _ = getUnspentOutputs account
    ...
    let collectInputs ((inputs, keys), collectedAmount) outpoint output =
        ...
        if isMature && amount > collectedAmount && asset = output.spend.asset then
            match output.lock with
            | Coinbase (_,hash)
            | PK hash when hash = PublicKey.hash account.publicKey ->
                let keyPair = secretKey, account.publicKey
                (((outpoint,output) :: inputs, keyPair :: keys), output.spend.amount + collectedAmount)
            | Coinbase (_,hash)
            | PK hash when hash = PublicKey.hash account.publicKeyChange ->
                let keyPair = secretChangeKey, account.publicKeyChange
                (((outpoint,output) :: inputs, keyPair :: keys), output.spend.amount + collectedAmount)
            | _ -> failwith "No other spendable output found"
        else
        ...
        let changeOutput =
            {
                ...
                lock = PK (PublicKey.hash account.publicKeyChange)
            }
...
let createTransactionFromLock chainParams lk spend (account, extendedKey) = result {
    let! metavcoinKey = derivemetavcoinKey extendedKey
    let! secretKey = ExtendedKey.getPrivateKey metavcoinKey
    let! metavcoinChaingeKey = deriveChangemetavcoinKey extendedKey
    let! secretChangeKey = ExtendedKey.getPrivateKey metavcoinChaingeKey

    let! (inputs, keys, inputAmount) = collectInputs chainParams account spend.amount spend.asset secretKey secretChangeKey
...
let createActivationTransactionFromContract chainParams (_, ({queries=queries;code=code} as contractV0)) (numberOfBlocks:uint32) (account, extendedKey) =
    ...
    result {
        ...
        let! (inputs,keys,inputAmount) = collectInputs chainParams account (activationSacrifice + activationFee) Asset.metavcoin secretKey secretChangeKey
        let inputPoints = List.map (fst >> Outpoint) inputs

        let outputs =
            [
                { spend = { amount = activationSacrifice; asset = Asset.metavcoin }; lock = ActivationSacrifice }
                { spend = { amount = activationFee; asset = Asset.metavcoin }; lock = Fee }
            ]
            |> addChange inputAmount Asset.metavcoin account
...
let addReturnAddressToData publicKey data =
    let addReturnAddressToData' dict =
        ...
        metavcoin.Dictionary.add "returnAddress"B (ZData.Lock (ZFStar.fsToFstLock returnAddress)) dict
        |> Cost.__force
        |> ZData.Dict
        |> ZData.Collection
        |> Some
...
let createExecuteContractTransaction chainParams executeContract (contractId:ContractId) command data provideReturnAddress sign spends (account, extendedKey) = result {
    ...
    if Map.isEmpty spends then
        ...
        let! inputs, keys', collectedAmount =
            collectInputs chainParams account tempFeeAmount metavcoin secretKey secretChangeKey
        let inputs = List.map TxSkeleton.Input.PointedOutput inputs

        txSkeleton <-
            TxSkeleton.addInputs inputs txSkeleton
            |> TxSkeleton.addChange metavcoin collectedAmount tempFeeAmount pkHash

        let feeOutput = { lock = Fee; spend = { amount = tempFeeAmount; asset = metavcoin } }
        ...
        for (asset, amount) in Map.toSeq spends do
            let! inputs, keys', collectedAmount =
                collectInputs chainParams account amount asset secretKey secretChangeKey
            let inputs = List.map TxSkeleton.Input.PointedOutput inputs
