...
let createWalletFromInputs (outputs: List<Output>) (numOfInputs: uint64) : Contract.ContractWallet =
    let accumlatedSpend =
        ...
        |> Fund.find Asset.metavcoin

    [0UL .. numOfInputs - 1UL]
    |> List.map (fun (i) -> { txHash=Hash.zero; index=uint32 i }, {spend= {asset=Asset.metavcoin; amount=accumlatedSpend / numOfInputs}; lock = Contract cgpContractId})
...
let executeCgpContract (txSkeleton : TxSkeleton.T) context messageBody wallet =
    result {
        ...
        let! (txSkel,_,_) = Contract.run contract txSkeleton context command metavcoin.Types.Main.sender.Anonymous messageBody wallet None
...
let cgp1 : CGP.T =
    {
        allocation = 32uy
        payout     = Some (ContractRecipient cgpContractId, [ {asset=Asset.metavcoin; amount=123UL} ])
    }
#if DEBUG
[<Test>]
let ``valid payout`` () =
...
[<Test>]    
let ``valid payout, 2 asset metavcoin`` () =
...
let ``different winner asset`` () =
    ...
    let cgp : CGP.T =
        {
            allocation = 12uy
            payout     = Some (PKRecipient Hash.zero, [ { asset=Asset.metavcoin; amount=20UL } ])
        }
...
let ``A CGP execution with a higer then the limit is still a valid tx`` () =
    ...
    result {
        ...
        let coinbaseOutputs =
             [ { lock = Coinbase (context.blockNumber - 501u, account.publicKey |> CryptoPublicKey.hash); spend = {asset = Asset.metavcoin; amount = 4400009026UL;}; }
             ; { lock = Contract cgpContractId; spend = { asset = Asset.metavcoin; amount = 600000000UL;}; }
        ...
        let outputs =
            [ { lock = account.publicKey |> CryptoPublicKey.hash |> PK ; spend = { asset=Asset.metavcoin; amount=200000UL } }
            ; { lock = account.publicKey |> CryptoPublicKey.hash |> PK ; spend = { asset=Asset.metavcoin ; amount=123000000UL } }
            ]
        ...
        let executeContract _ _ _ (messageBody: metavcoin.Types.Data.data option) (txSkel : TxSkeleton.T) =
            let context = { context0 with blockNumber = 10u * chainParams.intervalLength + chainParams.coinbaseMaturity }
...
let ``valid connect with over the size limit CGP execution`` ()  =
    ...
    let res = executeCgpContractPerInputsInContext [ {lock=PK Hash.zero ; spend = { asset=Asset.metavcoin; amount=20_000_000UL } } ] context 1_000_000UL
    ...
    let cgp : CGP.T =
        {
            allocation = 12uy
            payout     = Some (PKRecipient Hash.zero, [ { asset=Asset.metavcoin; amount=20_000_000UL } ])
        }
...
let ``connect with invalid coinbase - CGP output 1st (instead of coinbase)`` ()  =
    ...
    let coinbaseOutputs =
         [ {lock = Contract cgpContractId; spend = {asset = Asset.metavcoin; amount = 600000000UL;};}
         ; {lock = Coinbase (context.blockNumber ,Hash.zero); spend = {asset = Asset.metavcoin; amount = 4400009026UL;};}
         ]
...
let ``connect with invalid coinbase - CGP overflow`` ()  =
    ...
    let coinbaseOutputs =
         [ {lock = Coinbase (context.blockNumber ,Hash.zero); spend = {asset = Asset.metavcoin; amount = 4400009026UL;};}
         ; {lock = Contract cgpContractId; spend = {asset = Asset.metavcoin; amount = 600000000UL + 1UL;};}
         ]
...
let ``connect with invalid coinbase - miner overflow`` ()  =
    ...
    let coinbaseOutputs =
         [ {lock = Coinbase (context.blockNumber ,Hash.zero); spend = {asset = Asset.metavcoin; amount = 4400009026UL + 1UL;};}
         ; {lock = Contract cgpContractId; spend = {asset = Asset.metavcoin; amount = 600000000UL;};}
         ]
...
let ``coinbase creation - non-CGP contract`` () =
    ...
    let coinbaseOutputs =
         [ {lock = Coinbase (context.blockNumber ,Hash.zero); spend = {asset = Asset.metavcoin; amount = 4400009026UL;};}
         ; {lock = Contract chainParams.votingContractId ; spend = {asset = Asset.metavcoin; amount = 600000000UL;};}
         ]
...
let ``implementation execution of the CGP contract`` () =
    ...
    result {
        ...
        let utxos =
            Map.empty
            |> Map.add {txHash=Transaction.hash coinbaseTx.tx; index=0u} (uns { lock = Coinbase (context.blockNumber - 501u, account.publicKey |> CryptoPublicKey.hash); spend = {asset = Asset.metavcoin; amount = 4400009026UL;}; })
            |> Map.add {txHash=Transaction.hash coinbaseTx.tx; index=1u} (uns { lock = Contract cgpContractId; spend = { asset = Asset.metavcoin; amount = 600000000UL;}; })
