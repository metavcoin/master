...
let ``Transaction with many PK locks in inputs should have right cost``() =
    ...
    let outputs =
        [ for lk in outputLocks ->
            { lock = lk; spend = { asset = Asset.metavcoin; amount = 1UL} } ]
...
let ``Contract validated transaction should have the right cost``() =
    ...
    let cSpend = { asset = Asset.metavcoin; amount = 1UL }
    ...
    let outputSpends = [
        ...
        {asset=cAsset;amount=50UL};
        {asset=Asset.metavcoin;amount=10UL}
    ]
    let outputs = [
...
let ``Transaction with large amount of inputs should not fail weight calculation``() =
    ...
    for i in [ 1u .. size ] do
        ...
        let output = { lock = publicKey |> PublicKey.hash |> PK ; spend = { asset = Asset.metavcoin; amount = 1UL } }
    ...
    let output = { lock = publicKey |> PublicKey.hash |> PK ; spend = { asset = Asset.metavcoin; amount = 1UL * (uint64 size) } }
    let tx = {
        version = Version0
        inputs = inputs
        witnesses = []
