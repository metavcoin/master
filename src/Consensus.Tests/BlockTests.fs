...
let ``block with empty transactions failed validation``(header) =
    ...
    BlockValidation.validate block chain = Error "transactions is empty"
...
let ``block with invalid header failed validation``(header:BlockHeader) (NonEmptyTransactions transactions) =
    ...
    BlockValidation.validate block chain = Error "proof of work failed"
...
let ``block with one invalid transaction fail validation``(header) (NonEmptyTransactions transactions) (NonNegativeInt index) =
    ...
    BlockValidation.validate block chain = expected
...
let ``block with valid transactions pass validation``(header:BlockHeader) (NonEmptyTransactions transactions) =
    ...
    BlockValidation.validate block chain = Ok block
...
let ``connecting block failed when block number is not successive``(parent:BlockHeader) (block:Block) =
    ...
    ==>
    (match BlockConnection.connect state env with
    | Ok _ -> Ok ()
...
let ``block timestamp too early``() =
    ...
    let tx =
        TestWallet.createTransaction testParameters (publicKeyHash account1) {asset=Asset.metavcoin;amount=1UL} rootAccount
    ...
    let contract : Contract.T =
        {
            ...
            mainFn = fun tx _ _ _ _ _ _ _ -> Ok (tx,None,metavcoin.Types.Main.stateUpdate.NoChange)
    ...
    let tx =
        {
            ...
            outputs=[{lock=Coinbase (15ul,Hash.zero);spend={amount=1UL;asset=Asset.metavcoin}}]
            witnesses=[witness]
            contract=None
        }
...
let ``coinbase lock have wrong blockNumber``() =
    let tx =
        {
            version = Version0
            inputs = [];
            outputs=[{lock= Coinbase (13ul,Hash.zero);spend={amount=1UL;asset=Asset.metavcoin}}]
            witnesses=[]
            contract=None
        }
    ...
    //     Error  "Block failed coinbase validation due to General \"within coinbase transaction all outputs must use coinbase or contract lock\""

    BlockValidation.validate block chain
    |> expectGenericError
    ...
    let tx =
        {
            version = Version0
            inputs = [];
            outputs=[{lock= PK Hash.zero;spend={amount=1UL;asset=Asset.metavcoin}}]
            witnesses=[]
            contract=None
        }
    ...
    let tx =
        {
            ...
            outputs=
                [
                    {lock= Coinbase (15ul,Hash.zero);spend={amount=blockReward 15ul 0uy;asset=Asset.metavcoin}}
    ...
    let tx =
        {
            ...
            outputs=
                [
                    {lock= Coinbase (15ul,Hash.zero);spend={amount=(blockReward 15ul 0uy) / 2UL;asset=Asset.metavcoin}}
                    {lock= Coinbase (15ul,Hash.zero);spend={amount=(blockReward 15ul 0uy) / 2UL;asset=Asset.metavcoin}}
                ]
            witnesses=[]
            contract=None
...
let ``block spending mature transaction is valid``() =
    ...
    let origin =
        {
            version = Version0
            inputs=[]
            outputs=[{lock =  Coinbase (1ul, (publicKeyHash rootAccount)); spend= {asset = Asset.metavcoin;amount=100000000UL}}]
            witnesses=[]
            contract=None
        }
    ...
    let tx =
        TestWallet.createTransaction testParameters (publicKeyHash account1) { asset = Asset.metavcoin; amount = 1UL } (rootAccount, rootExtendedKey)
        |> (fun x -> match x with | Ok x -> x | _ -> failwith "failed transaction generation")
...
//    }
//
//    let spend = { asset = Asset.metavcoin; amount = 1UL }
//    let tx1 =
...
//
//    let res = BlockConnection.connect chain getUTXO contractsPath parent timestamp utxoSet acs ContractCache.empty ema getContractState ContractStates.asDatabase block
