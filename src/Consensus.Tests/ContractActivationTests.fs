...
let ``Contract activation with too low contract activation sacrifice``() =
    ...
    let outputs =
        ...
        [
            {lock=ActivationSacrifice;spend={amount=1UL;asset=Asset.metavcoin}}
...
let ``Contract extension with too low contract extension sacrifice``() =
    ...
    let outputs =
        ...
        [
            {lock=ExtensionSacrifice sampleContractId;spend={amount=1UL;asset=Asset.metavcoin}}
            {output with spend={output.spend with amount = output.spend.amount - 1UL}}
        ]
...
let ``Contract extension of a non active contract should fail``() =
    ...
    let outputs =
        ...
        [
            {lock=ExtensionSacrifice contractId;spend={amount=1UL;asset=Asset.metavcoin}}
            {output with spend={output.spend with amount = output.spend.amount - 1UL}}
        ]
...
[<Test>]
[<Parallelizable>]
let ``Contract activation with asset other than metavcoin should fail``() =
    ...
    let expected:TxResult = General "Sacrifice must be paid in metavcoin" |> Error
...
[<Test>]
[<Parallelizable>]
let ``Contract extension with asset other than metavcoin should fail``() =
    let asset = Asset (ContractId (Version0, Hash.compute "1"B),Hash.zero)
...
let ``Contract extension with exact amount``() =
    ...
    let outputs =
        ...
        [
            {lock=ExtensionSacrifice contractId;spend={amount=extensionSacrifice;asset=Asset.metavcoin}}
...
let ``Contract extension with more than one output``() =
    ...
    let outputs =
        ...
        [
            {lock=ExtensionSacrifice contractId;spend={amount=extensionSacrifice1;asset=Asset.metavcoin}}
            {lock=ExtensionSacrifice contractId;spend={amount=extensionSacrifice2;asset=Asset.metavcoin}}
...
let ``Contract activation without hints should fail``() =
    ...
    let outputs =
        ...
        [
            {lock=ActivationSacrifice;spend={amount=activationSacrificeAmount;asset=Asset.metavcoin}}
