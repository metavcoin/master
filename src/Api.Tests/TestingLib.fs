...
module Node =
    ...
    let init (index : int) (mine : NodeStatus) (wipe : WipeStatus) (dataPath:string) : System.Diagnostics.Process =
        ...
        let metavcoin_node_dir = System.IO.Path.Combine([| ".." ;"Node"; "bin"; BUILD |])

        let owd = System.IO.Directory.GetCurrentDirectory()

        System.IO.Directory.SetCurrentDirectory(metavcoin_node_dir)

        let args = String.concat " " [| "metavcoin-node.exe"; "--local"; string index; wipeString wipe; mineString mine; sprintf "--data-path \"%s\"" dataPath |]
...
module Contract =
    ...
    module MessageBody =
        ...
        let createVoteData index ballot blockNumber : byte array =
            ...
            metavcoin.Dictionary.add ballotString (ZData.String ballotSer) signatures
            |> Cost.__force
            |> ZData.Dict
            |> ZData.Collection
