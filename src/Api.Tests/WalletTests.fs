...
[<Test>]
let ``Should get metavcoin public key``() =
    use actors = getBasicActors()
    let client = ServiceBus.Client.create Utils.busName
    let config = {config with client = client }
    let contentConfig e = contentConfig e |> fun conf -> {conf with client = client }
    ...
    Api.Server.Wallet.metavcoinPublicKey (contentConfig expectedContent)
[<Test>]
let ``Should remove and import wallet``() =
    ...
    Api.Server.Wallet.mnemonicphrase (contentConfig expectedContent) body
[<Test>]
let ``Should fail reimport metavcoin public key``() =
    use actors = getBasicActors()
    let client = ServiceBus.Client.create Utils.busName
    let config = {config with client = client }
    ...
    let body = Constants.metavcoinPublicKey

    Api.Server.Wallet.importmetavcoinPublicKey (badRequestConfig expectedContent) body
    
//[<Test>]
//let ``Should fail sending from imported metavcoin public key``() =
//    use actors = getBasicActors()
//    let client = ServiceBus.Client.create Utils.busName
//    let config = {config with client = client }
//    let contentConfig e = contentConfig e |> fun conf -> {conf with client = client }
//    
//    let expectedContent = TextContent "metavcoinKey imported"
...
//
//    Api.Server.Wallet.importmetavcoinPublicKey (contentConfig expectedContent) body
//
