...
module CheckPlatform =
    ...
    let check =
        ...
        if Platform.platform = PlatformID.Win32NT && not Platform.is64bit  then
            System.Environment.FailFast("metavcoin-node can only run on 64 bit OS", null)
...
let main argv =
    ...
    let parser = ArgumentParser.Create<Argument>(programName = "metavcoin-node.exe", errorHandler = errorHandler)
